...
class HourglassFan(nn.Module):
    ...
    Args:
        depth: number of down/up levels (n in original code, 4 in FAN2D).
...
class Fan2D(PoseModel):
    """PyTorch port of the FAN2D network (ICCV 2017).

    This mirrors the structure of the original Torch7 implementation:
    - Initial stem: 7x7/2 conv -> BN -> ReLU -> Residual(64,128) -> pool ->
      Residual(128,128) -> Residual(128,nFeats).
    - nStacks stacked hourglass modules with nModules residuals and
      intermediate supervision.
    - Each stack produces a 68-channel heatmap prediction.

    Returns:
        last_heatmap, all_heatmaps (list of length nStacks).
    """

    def __init__(
        self,
        num_stacks: int = 8,   # nStack in original paper
